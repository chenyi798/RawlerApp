...
class EastMoneyCrawler:
    def __init__(self, log_callback=None):
        ...
        # 全局配置 - 增加延迟时间避免403
        self.config = {
            'min_delay': 5,  # 最小延迟时间(秒) - 从1增加到5
            'max_delay': 15,  # 最大延迟时间(秒) - 从3增加到15
            'max_retries': 3,  # 最大重试次数
            'timeout': 15,  # 请求超时时间(秒)
            'page_delay_min': 5,  # 页面请求最小延迟
            'page_delay_max': 15,  # 页面请求最大延迟
            'article_delay_min': 5,  # 文章详情请求最小延迟
            'article_delay_max': 15,  # 文章详情请求最大延迟
        }
    ...
    def get_articles_list(self, keyword, page_index=1, page_size=10, retry_count=0):
        ...
        try:
            ...
            if retry_count < self.config['max_retries']:
                retry_delay = random.uniform(5, 10)  # 重试延迟增加到5-10秒
    ...
    def get_all_articles(self, keyword):
        ...
        for page in range(2, total_pages + 1):
            ...
            # 增加页面请求延迟，避免403
            delay = random.uniform(self.config['page_delay_min'], self.config['page_delay_max'])
    ...
    def download_image_to_memory(self, img_url, retry_count=0):
        ...
        try:
            ...
            # 重试机制
            if retry_count < self.config['max_retries']:
                retry_delay = random.uniform(3, 8)  # 重试延迟增加到3-8秒
        ...
        try:
            ...
            for element in content_elements:
                ...
                if element['type'] == 'text':
                    ...
                    # 图片下载前增加延迟
                    img_delay = random.uniform(2, 5)  # 增加到2-5秒
                    self.log(f"等待{img_delay:.1f}秒后下载图片...")
    ...
    def process_articles(self, articles, keyword, save_dir):
        ...
        try:
            for i, article in enumerate(articles):
                ...
                # 显示处理进度
                if (i + 1) % 5 == 0 or (i + 1) == total_articles:  # 每5篇显示一次进度
                ...
                # 在请求文章详情前增加延迟，避免403
                if i > 0:  # 第一篇文章不需要延迟
                    delay = random.uniform(self.config['article_delay_min'], self.config['article_delay_max'])
                    self.log(f"等待{delay:.1f}秒后请求文章详情...")
                    time.sleep(delay)
                ...
                if i < total_articles - 1 and self.is_crawling:
                    delay = random.uniform(3, 8)  # 增加到3-8秒
